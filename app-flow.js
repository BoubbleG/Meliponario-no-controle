...
fetch('./data.json',{cache:'no-store'}).then(r=>r.json()).then(d=>{DATA=d;renderShell();renderHome();renderColoniesPage();bind();divisionMode='flow';divisionStep=1;setPage('division')}).catch(err=>{console.error(err);toast('Não foi possível carregar os dados de demonstração.')});
